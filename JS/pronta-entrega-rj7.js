    ...
    if( !executou ){
      ...
      $.when(cache1, cache2, cache3).done(function(data1, data2, data3){
        ...
        if($("body").hasClass('home')){
                ...
                var n = $(".prateleira").length;
                var item = "";
                for(i = 0; i < n; i++){
                  $(".prateleira").eq(i).attr('data-flag', 'prateleira-'+i);

                }

                $(".prateleira[data-flag='prateleira-0'] li span").each(function(){
                  if( ids.search($(this).attr('data-id')) >= 1){
                    $(this).parent().find('.wrapFlag').after('<div id="flag-pronta-entrega"></div>');
                  }
                });

                $(".prateleira[data-flag='prateleira-1'] li span").each(function(){
                  if( ids.search($(this).attr('data-id')) >= 1){
                    $(this).parent().find('.wrapFlag').after('<div id="flag-pronta-entrega"></div>');
                  }
                });
