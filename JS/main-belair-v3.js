    ...
    newsletter: function(){
        ...
        $(document).ajaxStop(function(){
        	...
        	var textResposta = '<h4>Seu email foi cadastrado com sucesso</h4>\
        						<p>Salve o código abaixo. Ele será necessário para a aplicação do desconto.</p>\
        						<strong class="cupom">Desc52015</strong></h3> <p>Na página de pagamento, digite o código no campo “cupom de desconto”\
								para receber 5% de desconto na sua primeira compra.</p> <p><em>Válida até 31/12/2015</em></p>'
...
var product = {
    ...
    inCashPrice: function () {
        ...
        var p = fns.numberWithCommas(pClubrNumString[0]);
       	...
       	$(".inCashPrice").html('ou R$ ' + p + ',' + pClubrNumString[1] + ' de 1x a 6x no <span class="cartao-area-nome"></span></b>');
    },

    applyDiscount: function () {
    	// 1x a 4x no cartão = 5% de desconto
		// 5 a 6x no cartao = 4% de desconto
		// 7x a 10x = preço do comum
		function calc (v) {			
	    	for (var i = 1; i <= 10; i++) {
	    		if (v >= 1 && v < 7) {
    				return 0.95;
...
$(document).ajaxStop(function () {
	var el = $('.freight-values td:contains("Frete Própria,")')
	el.text(el.text().replace('Frete Própria,',''))
...
$(".table .totalizers-list tr:last-child td.info").text("Taxas");


// Compra Persistente //

	if($('body.product.teste-compra-1').length > 0){
