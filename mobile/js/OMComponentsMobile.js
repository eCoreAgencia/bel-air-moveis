var OMComponentsMobile = function(){
    ...
    */
    ...
    this.filter.validateSearch.validate   = function(){
        ...
        var option      = {type: 'GET'};
        var category    = vtxctx.categoryId == vtxctx.departmentyId? category = "" : category = vtxctx.categoryId + "/";
        var parameters  = "fq=C:/" + vtxctx.departmentyId+ "/" + category + parameters;
   ...
   */
   ...
   this.menu.appendBackButton = function(content){
       ...
       $(".menu-departamento .closeMenu").on("click", function(){
   ...
   */
    ...
    */
    ...
    $('.sku-selector-container').each(function(i){

        var $container = $(this);

        $(this).find("ul").each(function(j){
            
            var textSpecification   = $(this).find('.skuList input:first-child').attr('dimension')  //DESCRICAO DA VARIACAO DO SKU
            var specificationId     = textSpecification + '-' + i + '-' + j //DESCRICAO DA VARIACAO DO SKU
            
            function setSelectedSku (){
                var val = $container.find('#skuSelector'+ specificationId + ' input:checked').val()
                var cloneClass = $container.find('#skuSelector'+ specificationId + ' input:checked + label').attr('class')
                $container.find('#btnSkuSelector'+ specificationId + " .selected").text(val).addClass(cloneClass)
            } 
             
            var idLightbox = '#skuSelector' + specificationId; //ID PARA O LIGHTBOX

            //CRIA O LIGHTBOX
            var lightbox = $('<div>',{'id':'skuSelector'+specificationId,'class':'lightbox'})      
            ...
            $container.prepend($('<a>', {'id':'btnSkuSelector' + specificationId,'data-target':'skuSelector' + specificationId,'class':'skuSelectorAction','text':textSpecification}).append($('<span>',{'class':'selected'})))
            ...
            if($container.find('#skuSelector'+ specificationId + ' input').length == 1){
                setSelectedSku()
            }

            $container.find('#skuSelector'+ specificationId + ' input').on('click',function(){
                setSelectedSku()
            })
        ...
        $thumbsCloned = $('<div>', {'class': 'thumbsCloned'})
        ...
        setTimeout(function(){
            ...
            $thumbsCloned.html( $('.thumbs').html())    
            
            $thumbsCloned.appendTo($('.thumbs').parent())  
            $thumbsCloned = $('.thumbsCloned')
